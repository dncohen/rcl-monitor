...
class RCLMonitor extends EventEmitter {
  ...
  /**
   * Listen for transactions affecting the specified address.  When a
   * transaction is detected, an event is emitted with the passed in
   * context and the transaction.  An address cannot be added more
   * than once (the last call determines the context).
   */
        ...
        return me._ripple.getTransactions(address, options).then(txs => {
          if (txs.length > 0) {
            ...
            for (var i = 0; i < txs.length; i++) {
              me.emit('transaction', addressData['context'], txs[i], address);
            }
          }
